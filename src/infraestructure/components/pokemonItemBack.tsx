...
const PokemonItemBack = () => {
  ...
  const { fightingPokemons, pokemonSelected } = useSelector(
  ...
  const handlerFightingPokemon = () => {
    if (labelBtn === "Borrar de la lista") {
      const newFigthers = fightingPokemons.filter(
        (pokemon: PokemonsData) => pokemon.id !== Number(id)
      );
      // @ts-ignore
      ...
      if (fightingPokemons.length <= 6) {
        // @ts-ignore
        dispatch(SetFightingPokemons(
            [...fightingPokemons, pokemonAction],
            updateTokens
          )
        );
  ...
  return (
    <div className="backAdd">
      ...
      <button className="btnAdd" onClick={() => handlerFightingPokemon()}>
        {labelBtn}
      </button>
    </div>
  );
