...
const PokemonCard = ({
  ...
  useEffect(() => {
    const updateData = dataPokemons.filter(
      (pokemon: PokemonsData) => pokemon.id !== -1
    );
    setPokemons(updateData);
  }, [dataPokemons]);

  useEffect(() => {
    const filterData = dataPokemons.filter(
      (pokemon) =>
        pokemon.name
          .toUpperCase()
          .includes(filterPokemon?.toUpperCase() || "") &&
        pokemon.icon === IconType.add
    );
  ...
  const removeFightinhPokemon = (pokemonAction: PokemonsData) => {
    const newFigthers = dataPokemons.filter(
      (pokemon: PokemonsData) => pokemon.id !== pokemonAction.id
    );
    // @ts-ignore
  ...
  const showIcon = (pokemonLoad: PokemonsData): boolean => {
    const index =
      fightingPokemons?.findIndex(
        (pokemon: PokemonsData) => pokemon.name === pokemonLoad.name
      ) || 0;
    return pokemonLoad.icon === IconType.add && index > 0;
  };
  return (
    <div className="card">
      {pokemons.length === 0 && <h4>Ingrese un nombre de un pokemon válido</h4>}
      {pokemons.map((pokemon: PokemonsData) => (
        <li className="card_list" key={pokemon.id}>
          <div>
            {!showIcon(pokemon) ? (
              <i
                className={`fa-solid ${pokemon.icon}`}
                onClick={() => handlerFightingPokemon(pokemon)}
              ></i>
            ) : (
